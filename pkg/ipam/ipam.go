...
func (ipam *IPAM) GetRandomAddress(ctx context.Context, podName, nicName string, mac *string, subnetName, poolName string, skippedAddrs []string, checkConflict bool) (string, string, string, error) {
	...
	v4IP, v6IP, macStr, err := subnet.GetRandomAddress(ctx, poolName, podName, nicName, mac, skippedAddrs, checkConflict)
	if v4IP != nil {
		v4 = v4IP.String()
	}
	if v6IP != nil {
	...
	if poolName == "" {
		logger.Info("allocated from subnet", "ipv4", v4, "ipv6", v6, "mac", macStr, "pod", podName, "subnet", subnetName)
	} else {
		logger.Info("allocated from ip pool in subnet", "ipv4", v4, "ipv6", v6, "mac", macStr, "pod", podName, "subnet", subnetName, "pool", poolName)
	}
	return v4, v6, macStr, err
}

func (ipam *IPAM) GetStaticAddress(ctx context.Context, podName, nicName, ip string, mac *string, subnetName string, checkConflict bool) (string, string, string, error) {
	ipam.mutex.RLock()
	defer ipam.mutex.RUnlock()
	...
	var subnet *Subnet
	var ok bool
	logger.Info("allocating static ip from subnet", "ip", ip, "subnet", subnetName)
	...
	for _, ipStr := range strings.Split(ip, ",") {
		ip, err := NewIP(ipStr)
		if err != nil {
			logger.Error(err, "failed to parse ip", "ip", ipStr)
			return "", "", "", err
		}
		ipAddr, macStr, err = subnet.GetStaticAddress(podName, nicName, ip, mac, false, checkConflict)
		if err != nil {
			logger.Error(err, "failed to allocate static ip for pod", "ip", ipStr, "pod", podName)
			return "", "", "", err
		}
		ips = append(ips, ipAddr)
	}
	ips, err = checkAndAppendIpsForDual(ctx, ips, macStr, podName, nicName, subnet, checkConflict)
	if err != nil {
		logger.Error(err, "failed to append allocate ip mac %s for %s", ips, *mac, podName)
		return "", "", "", err
	}
...
func checkAndAppendIpsForDual(ctx context.Context, ips []IP, mac, podName, nicName string, subnet *Subnet, checkConflict bool) ([]IP, error) {
	// IP Address for dual-stack should be format of 'IPv4,IPv6'
	if subnet.Protocol != utils.ProtocolDual || len(ips) == 2 {
		return ips, nil
	}
	...
	var ipAddr IP
	var err error
	if utils.CheckProtocol(ips[0].String()) == utils.ProtocolIPv4 {
		newIps = ips
		_, ipAddr, _, err = subnet.getV6RandomAddress(ctx, "", podName, nicName, &mac, nil, checkConflict)
		newIps = append(newIps, ipAddr)
	} else if utils.CheckProtocol(ips[0].String()) == utils.ProtocolIPv6 {
		ipAddr, _, _, err = subnet.getV4RandomAddress(ctx, "", podName, nicName, &mac, nil, checkConflict)
		newIps = append(newIps, ipAddr)
...
func (ipam *IPAM) ReleaseAddressByPod(ctx context.Context, podName, subnetName string) {
	ipam.mutex.RLock()
	defer ipam.mutex.RUnlock()
	if subnetName != "" {
		if subnet, ok := ipam.Subnets[subnetName]; ok {
			subnet.ReleaseAddress(ctx, podName)
		}
	} else {
...
func (ipam *IPAM) AddOrUpdateSubnet(ctx context.Context, name, cidrStr, gw string, excludeIps []string) error {
	...
	var (
		...
		v4cidrStr, v6cidrStr, v4Gw, v6Gw string
		cidrs                            []*net.IPNet
	)
	...
	if subnet, ok := ipam.Subnets[name]; ok {
		...
		if err != nil {
			logger.Error(err, "failed to parse v4 exclude ips", "ips", v4ExcludeIps)
			return err
		}
		v6Reserved, err := NewIPRangeListFrom(v6ExcludeIps...)
		if err != nil {
			logger.Error(err, "failed to parse v6 exclude ips %v", "ips", v6ExcludeIps)
			return err
		}
		if (protocol == utils.ProtocolDual || protocol == utils.ProtocolIPv4) &&
			...
			firstIP, _ := utils.FirstIP(v4cidrStr)
			lastIP, _ := utils.LastIP(v4cidrStr)
			...
			for nicName, ip := range subnet.V4NicToIP {
				if !ips.Contains(ip) {
					...
					logger.Error(err, "pod v4 ip not in new cidr of subnet", "podName", podName, "ip", ip, "subnet", name, "cidr", cidrStr)
			...
			for nicName, ip := range subnet.V4NicToIP {
				logger.Info("already assigned v4 ip to nic in subnet", "ip", ip, "nicName", nicName, "subnet", name)
			}
		}
		if (protocol == utils.ProtocolDual || protocol == utils.ProtocolIPv6) &&
			...
			firstIP, _ := utils.FirstIP(v6cidrStr)
			lastIP, _ := utils.LastIP(v6cidrStr)
			ips, _ := NewIPRangeListFrom(fmt.Sprintf("%s..%s", firstIP, lastIP))
			...
			for nicName, ip := range subnet.V6NicToIP {
				if !ips.Contains(ip) {
					...
					logger.Error(err, "pod v6 ip not in new cidr of subnet", "podName", podName, "ip", ip, "subnet", name, "cidr", cidrStr)
			...
			for nicName, ip := range subnet.V6NicToIP {
				logger.Info("already assigned v6 ip to nic in subnet", "ip", ip, "nicName", nicName, "subnet", name)
			}
		}
	...
	if err != nil {
		logger.Error(err, "failed to create subnet", "subnet", name)
		return err
	}
	subnet.V4Gw = v4Gw
	subnet.V6Gw = v6Gw
	logger.Info("added new subnet", "subnet", name)
...
func (ipam *IPAM) ContainAddress(address string) (bool, error) {
	ipam.mutex.RLock()
	defer ipam.mutex.RUnlock()
...
func (ipam *IPAM) IsIPAssignedToOtherPod(ctx context.Context, ip, subnetName, podName string) (string, bool) {
	ipam.mutex.RLock()
	defer ipam.mutex.RUnlock()
	...
	return subnet.isIPAssignedToOtherPod(ctx, ip, podName)
}
