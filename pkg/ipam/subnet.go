...
func NewSubnet(name, cidrStr string, excludeIps []string) (*Subnet, error) {
	...
	excludeIps = utils.ExpandExcludeIPs(excludeIps, cidrStr)
	v4ExcludeIps, v6ExcludeIps := utils.SplitIpsByProtocol(excludeIps)
	...
	protocol := utils.CheckProtocol(cidrStr)
	subnet := &Subnet{
		Name:         name,
		CIDR:         cidrStr,
		Protocol:     protocol,
	...
	case utils.ProtocolIPv4:
		firstIP, _ := utils.FirstIP(cidrStr)
		lastIP, _ := utils.LastIP(cidrStr)
	...
	default:
		...
		v4FirstIP, _ := utils.FirstIP(cidrBlocks[0])
		v4LastIP, _ := utils.LastIP(cidrBlocks[0])
		v6FirstIP, _ := utils.FirstIP(cidrBlocks[1])
		v6LastIP, _ := utils.LastIP(cidrBlocks[1])
...
func (s *Subnet) GetStaticMac(podName, nicName, mac string, checkConflict bool) error {
	...
	if checkConflict {
		if p, ok := s.MacToPod[mac]; ok && p != podName {
			return fmt.Errorf("mac %s has been allocated to pod %s", mac, p)
		}
	}
	s.MacToPod[mac] = podName
	s.NicToMac[nicName] = mac
...
func (s *Subnet) pushPodNic(podName, nicName string) {
	if s.V4NicToIP[nicName] != nil || s.V6NicToIP[nicName] != nil || s.NicToMac[nicName] != "" {
		s.PodToNicList[podName] = utils.UniqString(append(s.PodToNicList[podName], nicName))
	}
}

func (s *Subnet) popPodNic(podName, nicName string) {
	s.PodToNicList[podName] = utils.RemoveString(s.PodToNicList[podName], nicName)
	if s.PodToNicList[podName] == nil {
		delete(s.PodToNicList, podName)
	}
}

func (s *Subnet) GetRandomAddress(ctx context.Context, poolName, podName, nicName string, mac *string, skippedAddrs []string, checkConflict bool) (IP, IP, string, error) {
	s.mutex.Lock()
	defer func() {
	...
	case utils.ProtocolDual:
		return s.getDualRandomAddress(ctx, poolName, podName, nicName, mac, skippedAddrs, checkConflict)
	case utils.ProtocolIPv4:
		return s.getV4RandomAddress(ctx, poolName, podName, nicName, mac, skippedAddrs, checkConflict)
	default:
		return s.getV6RandomAddress(ctx, poolName, podName, nicName, mac, skippedAddrs, checkConflict)
	}
}

func (s *Subnet) getDualRandomAddress(ctx context.Context, poolName, podName, nicName string, mac *string, skippedAddrs []string, checkConflict bool) (IP, IP, string, error) {
	v4IP, _, _, err := s.getV4RandomAddress(ctx, poolName, podName, nicName, mac, skippedAddrs, checkConflict)
	if err != nil {
		return nil, nil, "", err
	}
	_, v6IP, macStr, err := s.getV6RandomAddress(ctx, poolName, podName, nicName, mac, skippedAddrs, checkConflict)
	if err != nil {
		return nil, nil, "", err
	}
	...
	if !s.V4NicToIP[nicName].Equal(v4IP) {
		v4IP, _, _, _ = s.getV4RandomAddress(ctx, poolName, podName, nicName, mac, skippedAddrs, checkConflict)
	}

	return v4IP, v6IP, macStr, nil
}

func (s *Subnet) getV4RandomAddress(ctx context.Context, ippoolName, podName, nicName string, mac *string, skippedAddrs []string, checkConflict bool) (IP, IP, string, error) {
	...
	if s.V4NicToIP[nicName] != nil && s.NicToMac[nicName] != "" {
		if !utils.ContainsString(skippedAddrs, s.V4NicToIP[nicName].String()) {
			return s.V4NicToIP[nicName], nil, s.NicToMac[nicName], nil
		}
		s.releaseAddr(ctx, podName, nicName)
	}
	...
	if ip == nil {
		return nil, nil, "", fmt.Errorf("no free v4 ip in ip pool %s", ippoolName)
	}
...
func (s *Subnet) getV6RandomAddress(ctx context.Context, ippoolName, podName, nicName string, mac *string, skippedAddrs []string, checkConflict bool) (IP, IP, string, error) {
	// After 'macAdd' introduced to support only static mac address, pod restart will run into error mac AddressConflict
	// controller will re-enqueue the new pod then wait for old pod deleted and address released.
	// here will return only if both ip and mac exist, otherwise only ip without mac returned will trigger CreatePort error.
	if s.V6NicToIP[nicName] != nil && s.NicToMac[nicName] != "" {
		if !utils.ContainsString(skippedAddrs, s.V6NicToIP[nicName].String()) {
	...
	if ip == nil {
		return nil, nil, "", fmt.Errorf("no free v6 ip in ip pool %s", ippoolName)
	}
...
func (s *Subnet) GetStaticAddress(podName, nicName string, ip IP, mac *string, force, checkConflict bool) (IP, string, error) {
	...
	if v4 {
		if existPod, ok := s.V4IPToPod[ip.String()]; ok {
			...
			if !utils.ContainsString(pods, podName) {
				if !checkConflict {
					s.V4NicToIP[nicName] = ip
					s.V4IPToPod[ip.String()] = fmt.Sprintf("%s,%s", s.V4IPToPod[ip.String()], podName)
					return ip, macStr, nil
				}
				return ip, macStr, fmt.Errorf("ipv4 %s has been allocated to %v", ip.String(), pods)
			}
			if !force {
				return ip, macStr, nil
			}
		...
		if existPod, ok := s.V6IPToPod[ip.String()]; ok {
			...
			if !utils.ContainsString(pods, podName) {
				...
				return ip, macStr, fmt.Errorf("ipv6 %s has been allocated to %v", ip.String(), pods)
			}
			if !force {
				return ip, macStr, nil
			}
...
func (s *Subnet) releaseAddr(ctx context.Context, podName, nicName string) {
	logger := log.FromContext(ctx)
	logger.Info("releasing address", "pod", podName, "nic", nicName)
	...
	if ip, ok = s.V4NicToIP[nicName]; ok {
		...
		if len(oldPods) > 1 {
			newPods := utils.RemoveString(oldPods, podName)
			...
			if !s.V4CIDR.Contains(net.IP(ip)) {
				...
				logger.Info("release v4 address from subnet, ignore ip", "ip", ip, "mac", mac, "subnet", s.Name, "pod", podName)
				changed = true
			}

			if s.V4Reserved.Contains(ip) {
				logger.Info("release v4 address from subnet, ip is in reserved list", "ip", ip, "mac", mac, "subnet", s.Name, "pod", podName)
				changed = true
			}
				...
				if pool.V4Using.Remove(ip) {
					...
					if !changed {
						if pool.V4Released.Add(ip) {
							logger.Info("release v4 address from subnet, add ip to released list", "ip", ip, "mac", mac, "subnet", s.Name, "pod", podName)
						}
					}
					break
				}
	...
	if ip, ok = s.V6NicToIP[nicName]; ok {
		...
		if len(oldPods) > 1 {
			...
			if !s.V6CIDR.Contains(net.IP(ip)) {
				logger.Info("release v6 address from subnet, ignore ip", "ip", ip, "mac", mac, "subnet", s.Name, "pod", podName)
				changed = true
			}

			if s.V6Reserved.Contains(ip) {
				logger.Info("release v6 address from subnet, ip is in reserved list", "ip", ip, "mac", mac, "subnet", s.Name, "pod", podName)
				changed = true
			}
				...
				if pool.V6Using.Remove(ip) {
					...
					if !changed {
						if pool.V6Released.Add(ip) {
							logger.Info("release v6 address from subnet, add ip to released list", "ip", ip, "mac", mac, "subnet", s.Name, "pod", podName)
						}
					}
					break
				}
...
func (s *Subnet) ReleaseAddress(ctx context.Context, podName string) {
	s.mutex.Lock()
	defer s.mutex.Unlock()
...
func (s *Subnet) ReleaseAddressWithNicName(ctx context.Context, podName, nicName string) {
	s.mutex.Lock()
	defer s.mutex.Unlock()
...
func (s *Subnet) GetPodAddress(_, nicName string) (IP, IP, string, string) {
	...
	case utils.ProtocolIPv4:
		...
		return ip, nil, mac, utils.ProtocolIPv4
	case utils.ProtocolIPv6:
		ip, mac := s.V6NicToIP[nicName], s.NicToMac[nicName]
		return nil, ip, mac, utils.ProtocolIPv6
	default:
		v4IP, v6IP, mac := s.V4NicToIP[nicName], s.V6NicToIP[nicName], s.NicToMac[nicName]
		return v4IP, v6IP, mac, utils.ProtocolDual
	}
}

func (s *Subnet) isIPAssignedToOtherPod(ctx context.Context, ip, podName string) (string, bool) {
	logger := log.FromContext(ctx)
	if existPod, ok := s.V4IPToPod[ip]; ok {
		logger.V(4).Info("v4 check ip assigned", "existPod", existPod, "pod", podName)
		pods := strings.Split(existPod, ",")
		if !utils.ContainsString(pods, podName) {
	...
	if existPod, ok := s.V6IPToPod[ip]; ok {
		logger.V(4).Info("v6 check ip assigned", "existPod", existPod, "pod", podName)
		pods := strings.Split(existPod, ",")
		if !utils.ContainsString(pods, podName) {
			return existPod, true
		}
...
func (s *Subnet) AddOrUpdateIPPool(name string, ips []string) error {
	...
	var err error
	v4IPs, v6IPs := utils.SplitIpsByProtocol(ips)
	if s.V4CIDR != nil {
		...
		firstIP, _ := utils.FirstIP(s.V4CIDR.String())
		lastIP, _ := utils.LastIP(s.V4CIDR.String())
	...
	if s.V6CIDR != nil {
		...
		firstIP, _ := utils.FirstIP(s.V6CIDR.String())
		lastIP, _ := utils.LastIP(s.V6CIDR.String())
...
func (s *Subnet) IPPoolStatistics(ippool string) (
	v4Available, v4Using, v6Available, v6Using utils.BigInt,
