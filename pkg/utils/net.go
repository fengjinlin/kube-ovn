...
// #nosec G101
const vpcExternalNet = "ovs-vpc-external-network"
...
func IP2BigInt(ipStr string) *big.Int {
	...
	if CheckProtocol(ipStr) == ProtocolIPv4 {
...
func SubnetBroadcast(subnet string) string {
	_, cidr, _ := net.ParseCIDR(subnet)
	var length uint
	if CheckProtocol(subnet) == ProtocolIPv4 {
		length = 32
	} else {
		length = 128
	}
...
func GenerateRandomV4IP(cidr string) (string, error) {
	return genRandomIP(cidr, false)
}

func GenerateRandomV6IP(cidr string) (string, error) {
	return genRandomIP(cidr, true)
}

func genRandomIP(cidr string, isIPv6 bool) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("failed to generate random ip: %v", err)
	}
	t := big.NewInt(0).Add(IP2BigInt(ip), add)
	return fmt.Sprintf("%s/%d", BigInt2Ip(t), netMask), nil
}
...
func AppendGwByCidr(gateway, cidrStr string) (string, error) {
	...
	for _, cidr := range strings.Split(cidrStr, ",") {
		...
		if CheckProtocol(gateway) == ProtocolIPv4 {
			gwArray[0] = gateway
			gwArray[1] = gw
		} else {
			gwArray[0] = gw
...
func SplitIpsByProtocol(excludeIps []string) ([]string, []string) {
	...
	for _, ex := range excludeIps {
		...
		if CheckProtocol(ips[0]) == ProtocolIPv4 {
...
func GetIPAddrWithMask(ip, cidr string) string {
	var ipAddr string
	if CheckProtocol(cidr) == ProtocolDual {
...
func SplitStringIP(ipStr string) (string, string) {
	...
	case ProtocolDual:
		for _, ipTmp := range strings.Split(ipStr, ",") {
			if CheckProtocol(ipTmp) == ProtocolIPv4 {
				v4IP = ipTmp
			} else {
				v6IP = ipTmp
			}
...
func GatewayContains(gatewayNodeStr, gateway string) bool {
	// the format of gatewayNodeStr can be like 'kube-ovs-worker:172.18.0.2, kube-ovs-control-plane:172.18.0.3', which consists of node name and designative egress ip
...
func GetDefaultListenAddr() string {
	...
	if os.Getenv("ENABLE_BIND_LOCAL_IP") == "true" {
		...
		if len(podIps) == 1 {
			addr = podIps[0]
			if CheckProtocol(podIps[0]) == ProtocolIPv6 {
...
func FilterCIDR(cidr, protocol string) string {
	for _, c := range strings.Split(cidr, ",") {
		if CheckProtocol(c) == protocol {
			return c
		}
	}
