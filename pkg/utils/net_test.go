...
func TestGenerateRandomV4IP(t *testing.T) {
	...
	for _, c := range tests {
		t.Run(c.name, func(t *testing.T) {
			...
			if err != nil {
				ans, _ := GenerateRandomV4IP(c.cidr)
				if c.want != ans {
					t.Errorf("%v expected %v, but %v got",
						c.cidr, c.want, ans)
				}
				...
				ans, _ := GenerateRandomV4IP(c.cidr)
				if IPNets.Contains(net.ParseIP(ans)) {
					t.Errorf("%v expected %v, but %v got",
						c.cidr, c.want, ans)
				}
			}
...
func TestGatewayContains(t *testing.T) {
	tests := []struct {
		...
		{
			name:           "base",
			gatewayNodeStr: "kube-ovs-worker:172.18.0.2, kube-ovs-control-plane:172.18.0.3",
			gateway:        "kube-ovs-worker",
			want:           true,
		},
		{
			name:           "err",
			gatewayNodeStr: "kube-ovs-worker:172.18.0.2, kube-ovs-control-plane:172.18.0.3",
			gateway:        "kube-ovs-worker1",
			want:           false,
		},
		{
			name:           "formatDiff",
			gatewayNodeStr: "kube-ovs-worker, kube-ovs-control-plane:172.18.0.3",
			gateway:        "kube-ovs-worker",
			want:           true,
		},
	}
