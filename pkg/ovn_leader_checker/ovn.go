...
func doOvnLeaderCheck(cfg *Configuration, podName, podNamespace string) {
	if podName == "" || podNamespace == "" {
		klog.ErrorS(nil, "env variables POD_NAME and POD_NAMESPACE must be set")
		os.Exit(1)
	}
	if cfg == nil || cfg.KubeClient == nil {
		klog.ErrorS(nil, "preValidChkCfg: invalid cfg")
		os.Exit(1)
	}
...
func patchPodLabels(cfg *Configuration, cachedPod *corev1.Pod, labels map[string]string) error {
	...
	pod.Labels = labels
	patch, err := utils.GenerateStrategicMergePatchPayload(cachedPod, pod)
	if err != nil {
...
func getTSName(index int) string {
	if index == 0 {
		return consts.InterconnectionSwitch
	}
	return fmt.Sprintf("%s%d", consts.InterconnectionSwitch, index)
}
...
func getTSCidr(index int) (string, error) {
	...
	if len(podIps) == 1 {
		if utils.CheckProtocol(podIps[0]) == utils.ProtocolIPv6 {
			proto = utils.ProtocolIPv6
		} else {
			proto = utils.ProtocolIPv4
		}
	} else if len(podIps) == 2 {
		proto = utils.ProtocolDual
	}
